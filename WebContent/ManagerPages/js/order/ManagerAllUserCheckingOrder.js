...
//alert("333");
// 表格设置
$("#ManagerAllUserCheckingOrderInfo").datagrid(
{
	title : '租赁单信息',
	fit : true,
	fitColumns : true,
	toolbar:"#ManagerAllUserCheckingOrderListTb",
	columns : [ [
			{
				field : 'id',
				title : '订单编号',
			...
			singleSelect : true,
			url : "order/searchManagerAllUserCheckingOrder.ctrl",
...
/**审核订单*/
$("#ManagerAllUserCheckingOrderOn").click(function(){
	//alert("zzz");
	var selectedData = $("#ManagerAllUserCheckingOrderInfo").datagrid("getSelected");
	...
	//	alert(license);
		if(order_status_kind=="预定中"&&order_check_kind=="审核中"&&pay_status_kind=="已付款"){
		//	alert("ccc");
		//	alert(id);
		    var requestData={
		    		id:id
		  ...
		  //  alert(requestData);
			$.ajax({ 
				...
				url : "order/ManagerAllUserCheckingOrderOn.ctrl",
				async :false,
				dataType : "json",
				...
				//	alert("删除成功");
					...
					if(data.object){
						...
						//更新车辆状态
						setTimeout(function(){
							$("#ManagerAllUserCheckingOrderInfo").datagrid("reload");
						},3000);
 ...
 * 用户订单条件查询
 */
function ManagerAllUserCheckingOrderInfoSerch(){
	var isValid = $("#ManagerAllUserCheckingOrderSearchForm").form('validate');
	if(! isValid){
		return;
	}
	...
	form_id = $('#ManagerAllUserCheckingOrderSearchForm input[name="id"]').val();
	form_order_status_kind = $('#ManagerAllUserCheckingOrderSearchForm input[name="order_status_kind"]').val();
	form_order_check_kind = $('#ManagerAllUserCheckingOrderSearchForm input[name="order_check_kind"]').val();
	form_pay_status_kind = $('#ManagerAllUserCheckingOrderSearchForm input[name="pay_status_kind"]').val();
	...
	$('#ManagerAllUserCheckingOrderInfo').datagrid('load',queryParams);
}

/**
 * 重置查询表单
 */
function ManagerAllUserCheckingOrderInfoRedo(){
	//alert("222");
	$("#ManagerAllUserCheckingOrderSearchForm").form("clear");
	$('#ManagerAllUserCheckingOrderInfo').datagrid('load',{});
}
...
$("#orderEdit").click(function() {
	...
	if(selectedData != null){	//判断是否有选中
		$('#ManagerAllUserCheckingOrderForm').form('load',selectedData);
		
		$("#ManagerAllUserCheckingOrderWindow").window({
			title : '查看订单',
			iconCls : 'icon-edit',
			shadow:true
		}).window("open");
...
/** 表单取消按钮点击事件 */
$("#ManagerAllUserCheckingOrderFormUndo").click(function(){
	$('#ManagerAllUserCheckingOrderForm').form('clear');
	$("#ManagerAllUserCheckingOrderWindow").window("close");
});
...
/** 表单保存按钮点击事件 *//*
$("#ManagerAllUserCheckingOrderFormSave").click(function(){
	var id = $('#ManagerAllUserCheckingOrderForm input[name="id"]').val(); 
